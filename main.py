...
def print_ai_stats(ai):
    print(' -- ai (%s) statistics -- \n')
    l = len(ai.static_evals)
    s = 0.0
    for evl in ai.static_evals:
        s += evl
    avg = s/l
    print('+ number of moves played: %d' % l)
    print('+ average evaluation count: %d' % avg)
    print('+ details:')
    print(ai.static_evals)
